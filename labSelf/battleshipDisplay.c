...
void battleshipDisplay_runTest() {
  ...
  for (int8_t i = 0; i < 10; i++) {
    for (int8_t j = 0; j < 10; j++) {
      rowNum = j;
      columnNum = i;
      battleshipDisplay_drawSelectedArea(0);
      utils_msDelay(100);
      battleshipDisplay_drawFire(j % 3);
      battleshipDisplay_drawSelectedArea(1);
    }
  }
}
